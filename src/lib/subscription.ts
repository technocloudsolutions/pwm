...
export async function getUserSubscription(user: User): Promise<SubscriptionTier> {
  if (!user) return 'free';
...
export async function canAddPassword(userId: string, currentCount: number): Promise<boolean> {
...
export async function canAddPersonalInfo(userId: string, currentCount: number): Promise<boolean> {
  const subscription = await getUserSubscription({ uid: userId } as User);
  const features = getSubscriptionFeatures(subscription);
  return currentCount < features.maxPersonalInfos;
}

export type BooleanFeatures = Exclude<keyof SubscriptionFeatures, 'maxPasswords' | 'maxPersonalInfos'>;

export async function hasFeature(user: User, feature: BooleanFeatures): Promise<boolean> {
